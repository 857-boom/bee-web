...
import {
  pageApplication,
  removeApplication,
  batchRemoveApplication,
  addApplication,
  modifyApplication
} from '@/api/application/config'
export default {
  name: 'Index',
  components: {
    STable,
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        ...
        return pageApplication(requestParameters)
          .then(res => {
            return res.result
          })
      },
  ...
  methods: {
    ...
    handleRemove (record) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk () {
          removeApplication(record.id).then(res => {
            if (res.success) {
              // _this.refresh()
      ...
      if (e.key === '1') {
        if (!this.selectedRowKeys.length > 0) {
          ...
          this.$confirm({
            ...
            cancelText: '取消',
            onOk () {
              batchRemoveApplication(_this.selectedRowKeys).then(res => {
                if (res.success) {
                  _this.refresh()
                  _this.$message.success('删除成功')
                } else {
    ...
    handleOk () {
      ...
      form.validateFields((errors, values) => {
        if (!errors) {
          if (values.id > 0) {
            // 修改 e.g.
            modifyApplication(values).then(res => {
              if (res.success) {
                this.visible = false
